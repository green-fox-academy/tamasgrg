...
// We have bunnies standing in a line, numbered 1, 2, ... The odd bunnies (1, 3, ..)
  ...
  function calculateNumberOfEarsOfBunniesAgain(numBunnies: number): number {
    if (numBunnies == 1) {
      return 3;
    } else if (numBunnies % 2) {
      return 3 + calculateNumberOfEarsOfBunniesAgain(numBunnies - 1);
    } else {
      return 2 + calculateNumberOfEarsOfBunniesAgain(numBunnies - 1);
    }
  }
  
  console.log(calculateNumberOfEarsOfBunniesAgain(3));
  export{};